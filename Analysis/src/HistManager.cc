...
int HistManager::Init()
{
  ...
  for ( int i_det=0; i_det<8; i_det++ ) {
    ...
    h_Dhit_E_mB2[i_det] = new TH1F(Form("h_D%d_E_mB2",i_det+1), 
                                   ...
                                   nbin_E, bin_E[0], bin_E[1]);
    h_Dhit_E_mB12[i_det] = new TH1F(Form("h_D%d_E_mB12",i_det+1), 
                                    Form("h_D%d_E_mB12;Energy[MeV];# event",i_det+1),
                                    nbin_E, bin_E[0], bin_E[1]);
  ...
  const Int_t nbin_ang = 9;
  const Double_t bin_ang[2] = {-22.5,382.5};
...
int HistManager::DrawSingleDet() 
{
  ...
  ///*
  h_Dhit_E_mNO[0]->SetFillColor(kBlack);
  hs->Add(h_Dhit_E_mNO[0]);
  h_Dhit_E_mB12[0]->SetFillColor(kRed+2);
  hs->Add(h_Dhit_E_mB12[0]);
